    ...
    with dbt_client.session():
        ...
        while True:
            ...
            try:
                ...
                parsed_payload = parse_user_prompt_local(user_input, semantic_metadata)
                
                # 2. Check if it's a conversational or metadata question
                if parsed_payload["is_conversational"]:
                    print(f"\n🤖 Agent: {parsed_payload['chat_response']}")
                    continue
                
                # 3. Fallback to executing an actual Snowflake query if metrics are passed
                if not parsed_payload["metrics"]:
                    print("\n🤖 Agent: I recognized that as a data request, but couldn't map it to a specific metric. Could you rephrase?")
                    continue
                    
                df = query_semantic_layer(parsed_payload)
